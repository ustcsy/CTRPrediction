...
def model(x_ph, weight, bias):
	h2_nontext = tf.nn.relu(tf.matmul(tf.concat(1, [x_ph['number'], x_ph['label']]), weight['nontxt'])+bias['nontxt'])
	h3 = tf.nn.relu(tf.matmul(tf.concat(1, [h2_nontext, x_ph['text']]), weight['fea_eng'])+bias['fea_eng'])
	h4 = tf.nn.relu(tf.matmul(h3, weight['fea_sel1'])+bias['fea_sel1'])
	h5 = tf.nn.relu(tf.matmul(h4, weight['fea_sel2'])+bias['fea_sel2'])
	h6 = tf.nn.relu(tf.matmul(h5, weight['fea_sel3'])+bias['fea_sel3'])
	h7 = tf.nn.relu(tf.matmul(h6, weight['tra1'])+bias['tra1'])
	logits = tf.matmul(h7, weight['tra2'])+bias['tra2']
...
if __name__=="__main__":
	...
	weight = {
		'nontxt': tf.Variable(tf.random_normal([102, 102], stddev=0.001)),
		'fea_eng': tf.Variable(tf.random_normal([302, 1500], stddev=0.001)),
	...
	bias = {
		'nontxt': tf.Variable(tf.constant(0.001, shape=[102])),
